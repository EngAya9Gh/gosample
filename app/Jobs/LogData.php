        ...
        try {
            ...
            if (isset($this->task->id) && $logService->hasIntegration($this->task) && $this->task->is_blazma) {
                if ($this->status == 'picked up') {
                    $sampleCheck = Sample::where('task_id', $this->task->id)->where('is_blazma', 0)->pluck('barcode_id')->toArray();
                    if (!empty($sampleCheck)) {
                        $sampleTrackingUpdates = SampleTracking::whereIn('sample_id', $sampleCheck)->where('is_collected', 0)->get();
                        if (!empty($sampleTrackingUpdates)) {
                            foreach ($sampleTrackingUpdates as $sampleTrackingUpdate) {
                                $sampleTracking = SampleTracking::where('id', $sampleTrackingUpdate->id)->where('is_collected', 0)->first();
                                if (isset($sampleTracking->id)) {
                                    $sampleTracking->is_collected = true;
                                    $sampleTracking->task_id = $this->task->id;
                                    $sampleTracking->save();
                                    $sampleUpdate = Sample::where('barcode_id', $sampleTrackingUpdate->sample_id)->where('task_id', $this->task->id)->first();
                    ...
                    SampleTracking::where('task_id', $this->task->id)->delete();
                }
                $samples = Sample::where('task_id', $this->task->id)->where('is_blazma', 1)->distinct('barcode_id')->get();
                $driver = Driver::find($this->task->driver_id);
                $driver_name = null;
                $driver_id = null;
                if (isset($driver->id)) {
