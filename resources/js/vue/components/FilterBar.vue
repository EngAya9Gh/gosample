...
<template>
  <div class="bg-surface dark:bg-surface-dark-card rounded-2xl shadow-card border border-slate-100 dark:border-white/5 p-4 mb-4">
    <div class="flex items-center gap-2 mb-3">
      <i class="ri-equalizer-line text-primary-600 text-[15px]"></i>
      <h3 class="text-[13px] font-bold text-ink dark:text-slate-100">{{ title }}</h3>
      <span v-if="subtitle" class="text-[11px] text-slate-400 dark:text-slate-500">{{ subtitle }}</span>
      <slot name="title-extra"></slot>
    </div>

    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-x-3 gap-y-2.5">
      <slot></slot>
    </div>

    <div class="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-white/5">
