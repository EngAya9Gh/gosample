...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.moneyTransfer.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.money-transfers.update', [$moneyTransfer->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('driver') ? 'is-invalid' : '' }}" name="driver_id"
                        ...
                        @foreach ($drivers as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('driver_id') ? old('driver_id') : $moneyTransfer->driver->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="client_id"
                        ...
                        @foreach ($clients as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('client_id') ? old('client_id') : $moneyTransfer->client->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('from_location') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($from_locations as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('from_location_id') ? old('from_location_id') : $moneyTransfer->from_location->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('to_location') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($to_locations as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('to_location_id') ? old('to_location_id') : $moneyTransfer->to_location->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                        ...
                        @foreach (App\Models\MoneyTransfer::STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('status', $moneyTransfer->status) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number"
                        name="amount" id="amount" value="{{ old('amount', $moneyTransfer->amount) }}" step="0.01"
                        required>
                ...
                {{-- <div class="form-group">
                    <label class="required"
                        for="from_location_otp">{{ trans('cruds.moneyTransfer.fields.from_location_otp') }}</label>
                    <input class="form-control {{ $errors->has('from_location_otp') ? 'is-invalid' : '' }}" type="text"
                        name="from_location_otp" id="from_location_otp"
                        value="{{ old('from_location_otp', $moneyTransfer->from_location_otp) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('to_otp') ? 'is-invalid' : '' }}" type="text"
                        name="to_otp" id="to_otp" value="{{ old('to_otp', $moneyTransfer->to_otp) }}" required>
