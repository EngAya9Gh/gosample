...
@section('content')
    ...
    <div class="card modern-filter-card">
        <div class="card-header">
            <h4 class="card-title mb-0">{{ trans('translation.edit') }} {{ trans('translation.task.title_singular') }}</h4>
        </div>
            ...
            <form method="POST" action="{{ route('admin.tasks.update', [$task->id]) }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        ...
                        @endif
                        <small class="help-block text-muted">{{ trans('translation.task.fields.from_location_helper') }}</small>
                    </div>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        @endif
                        <small class="help-block text-muted">{{ trans('translation.task.fields.to_location_helper') }}</small>
                    </div>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        @if ($errors->has('billing_client'))
                            <div class="invalid-feedback">{{ $errors->first('billing_client') }}</div>
                        @endif
                        <small class="help-block text-muted">{{ trans('translation.task.fields.billing_client_helper') }}</small>
                    </div>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        @endif
                        <small class="help-block text-muted">{{ trans('translation.task.fields.driver_helper') }}</small>
                    </div>

                    <div class="col-lg-6 mb-3">
                        <label for="task_type">{{ trans('translation.task.fields.task_type') }}</label>
                    ...
                    <div class="col-lg-6 mb-3">
                        <label for="statuss">{{ trans('translation.task.fields.status') }}</label>
                    ...
                    <div class="col-lg-6 mb-3">
                        <label for="takasi">{{ trans('translation.task.fields.takasi') }}</label>
                        <select class="form-control {{ $errors->has('takasi') ? 'is-invalid' : '' }}" name="takasi"
                            ...
                            @foreach (App\Models\Task::TAKASI_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('takasi', 'NO') === (string) $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                        @if ($errors->has('takasi'))
                            <div class="invalid-feedback">{{ $errors->first('takasi') }}</div>
                        @endif
                    </div>
                </div>

                <div class="col-lg-12 d-flex justify-content-end flex-wrap mt-2" style="gap: 10px;">
                    <a href="{{ route('admin.tasks.index') }}" class="btn btn-reset mb-1">
                        {{ trans('global.cancel') }}
                    </a>
