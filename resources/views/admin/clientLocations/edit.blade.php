...
@section('content')
    <div class="card modern-filter-card">
        <div class="card-header">
            <h4 class="card-title mb-0">{{ trans('translation.edit') }} {{ trans('cruds.clientLocation.title_singular') }}</h4>
        </div>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($clients as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('client_id') ? old('client_id') : $clientLocation->client->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($locations as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('location_id') ? old('location_id') : $clientLocation->location->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
