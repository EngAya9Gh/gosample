...
@section('content')
    <div class="card modern-filter-card">
        <div class="card-header">
            <h4 class="card-title mb-0">{{ trans('translation.edit') }} {{ trans('translation.clientAccount') }}</h4>
        </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.client-accounts.update', [$clientAccount->id]) }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($clients as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('client_id') ? old('client_id') : $clientAccount->client->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <input class="form-control {{ $errors->has('username') ? 'is-invalid' : '' }}" type="text"
                            name="username" id="username" value="{{ old('username', $clientAccount->username) }}"
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" type="text"
                            name="password" id="password" value="{{ old('password', $clientAccount->password) }}"
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                            name="name" id="name" value="{{ old('name', $clientAccount->name) }}"
                    ...
                    <div class="col-lg-6 mb-3">
                        <label for="status">{{ trans('translation.clientAccount.fields.status') }}</label>
                        <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                            ...
                            @foreach (App\Models\ClientAccount::STATUS_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('status', $clientAccount->status) === (string) $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-lg-12 d-flex justify-content-end flex-wrap mt-2" style="gap: 10px;">
                    <a href="{{ route('admin.client-accounts.index') }}" class="btn btn-reset mb-1">
                        {{ trans('global.cancel') }}
                    </a>
