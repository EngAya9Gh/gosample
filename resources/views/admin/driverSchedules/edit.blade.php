...
@section('content')
    ...
    <div class="card modern-filter-card">
        <div class="card-header">
            <h4 class="card-title mb-0">{{ trans('translation.edit') }} {{ trans('translation.driverSchedule.title_singular') }}</h4>
        </div>
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('from_location') ? 'is-invalid' : '' }}"
                            name="from_locationd" id="from_location">
                            @foreach ($from_locations as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('from_locationd') ? old('from_locationd') : $driverSchedule->from->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('to_location') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($to_locations as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('to_location') ? old('to_location') : $driverSchedule->to->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('driver') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($drivers as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('driver_id') ? old('driver_id') : $driverSchedule->driver->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <input class="form-control {{ $errors->has('plate_number') ? 'is-invalid' : '' }}" type="text"
                            ...
                            value="{{ old('plate_number', $driverSchedule->plate_number) }}" placeholder="Vehicle plate">
                    ...
                    <div class="col-lg-12 mb-3">
                        ...
                        <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note"
                            id="note" rows="3" placeholder="Optional note">{{ old('note', $driverSchedule->note) }}</textarea>
                        @if ($errors->has('note'))
