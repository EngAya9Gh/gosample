...
@section('content')
    <div class="card modern-filter-card">
        <div class="card-header">
            <h4 class="card-title mb-0">{{ trans('translation.edit') }} {{ trans('cruds.carLinkHistory.title_singular') }}</h4>
        </div>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('driver') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($drivers as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('driver_id') ? old('driver_id') : $carLinkHistory->driver->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('car') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($cars as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('car_id') ? old('car_id') : $carLinkHistory->car->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select class="form-control {{ $errors->has('action') ? 'is-invalid' : '' }}" name="action"
                            ...
                            @foreach (App\Models\CarLinkHistory::ACTION_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('action', $carLinkHistory->action) === (string) $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
