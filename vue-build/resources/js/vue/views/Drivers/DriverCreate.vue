...
function submit(){ saving.value=true; setTimeout(()=>{saving.value=false; push({type:'success',title:'Saved',message:'Driver created'});},700); }
</script>
<template>
  <div>
    <Breadcrumb title="New Driver" :trail="[{label:'Drivers',href:'#/admin/drivers'},{label:'Create'}]" />
    <form @submit.prevent="submit" class="max-w-3xl space-y-5">
      <BaseCard title="Identity & login" icon="ri-user-line">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
          ...
          <FormInput v-model="f.password" label="Password" type="password" :required="true" />
