...
<template>
  <header class="relative z-40 flex items-center gap-2 h-16 px-4 bg-surface dark:bg-slate-900/80 backdrop-blur border-b border-slate-200/70 dark:border-white/5">
    <!-- hamburger -->
    <button @click="$emit('toggle-sidebar')" class="grid place-items-center w-10 h-10 rounded-xl text-slate-500 hover:bg-surface-muted dark:hover:bg-white/10 transition">
      <i class="ri-menu-2-line text-xl"></i>
    </button>

    <!-- quick create -->
    <div class="relative">
      <button @click="toggle('create')" class="inline-flex items-center gap-2 h-10 px-3.5 rounded-xl bg-primary-700 text-white text-sm font-medium shadow-card hover:bg-primary-800 transition">
        <i class="ri-add-line text-base"></i><span class="hidden sm:inline">Create</span>
      </button>
      <Transition name="drop">
        <div v-if="openMenu === 'create'" class="absolute mt-2 inset-inline-start-0 w-60 bg-surface dark:bg-slate-800 rounded-xl shadow-card-hover border border-slate-100 dark:border-white/10 p-1.5" style="inset-inline-start:0">
    ...
    <div class="ms-auto flex items-center gap-1">
      <!-- lost samples -->
      <button @click="$emit('navigate', '/admin/lost')" class="relative grid place-items-center w-10 h-10 rounded-xl text-slate-500 hover:bg-surface-muted dark:hover:bg-white/10 transition" title="Lost samples">
      ...
      <!-- notifications -->
      <div class="relative">
        <button @click="toggle('notif')" class="relative grid place-items-center w-10 h-10 rounded-xl text-slate-500 hover:bg-surface-muted dark:hover:bg-white/10 transition">
      ...
      <!-- dark -->
      <button @click="$emit('toggle-dark')" class="grid place-items-center w-10 h-10 rounded-xl text-slate-500 hover:bg-surface-muted dark:hover:bg-white/10 transition" title="Toggle theme">
